...
│                 │      │                       │    │                         │
│ clickstream_    │      │  olist_db             │    │  raw-reviews/           │
...
│                         │   │                              │
│  Features:              │   │  Features (RFM):             │
│  lag_1, lag_7, lag_14   │   │  recency, frequency,         │
│  rolling_7d_mean        │   │  monetary, avg_rating        │
│  day_of_week, is_weekend│   │                              │
│  month, quarter         │   │  Label: churned=1 se         │
│                         │   │  recency > 30 dias           │
│  Modelo:                │   │                              │
│  LinearRegression       │   │  Modelo:                     │
│  (Spark MLlib)          │   │  RandomForest (Spark MLlib)  │
│                         │   │                              │
│  Avaliação:             │   │  Avaliação:                  │
│  RMSE, MAE              │   │  F1, AUC-ROC                 │
│  (últimos 20% das datas)│   │  (80/20 random split)        │
└────────────┬────────────┘   └───────────────┬──────────────┘
             │   ↕ MLflow (métricas + modelo)  │
             ▼                                 ▼
┌────────────────────────┐   ┌──────────────────────────────┐
│ lake.gold.             │   │ lake.gold.                   │
│ ml_demand_forecast     │   │ ml_churn_scores              │
│                        │   │                              │
│ category_id            │   │ customer_id                  │
│ category_en            │   │ churn_probability            │
│ forecast_date          │   │ risk_label (high/medium/low) │
│ predicted_orders       │   │ recency, frequency, monetary │
│ model_rmse, model_mae  │   │ model_f1, model_auc          │
│ scored_at              │   │ scored_at                    │
└────────────────────────┘   └──────────────────────────────┘
             │                              │
             ╚══════════════╦═══════════════╝
...
Evento gerado pelo simulador
    ...
    Trino Views actualizadas após cada run
         │
         ▼
    ML Gold Tables actualizadas 1x/dia às 03:00 (demand forecast + churn scores)
         │
         ▼
...
├──────────────────┼──────────────────────────────────────────────────┤
│ ML — Modelos     │ Apache Spark MLlib 3.5 (LinearRegression, RF)    │
